...
class MedicalController
{
    ...
    public function hospitals(): void
    {
        ...
        $createUrl = has_role('admin') ? url('?page=hospital-create') : null;
        $this->page('hospitals', 'Hôpitaux', $rows, ['nom' => 'Établissement', 'adresse' => 'Adresse', 'telephone' => 'Téléphone', 'lits' => 'Lits', 'litsOccupes' => 'Occupés', 'occupation' => 'Occupation %', 'services' => 'Services'], $createUrl, 'Nouvel hôpital');
    }
    ...
    {
        if (has_role('patient')) {
            $stmt = db()->prepare('SELECT c.*, CONCAT(p.prenom, " ", p.nom) patient, u.name medecin FROM consultations c JOIN patients p ON p.id=c.patientId JOIN medecins m ON m.id=c.medecinId JOIN users u ON u.id=m.userId WHERE p.userId = ? ORDER BY c.dateConsultation DESC');
            ...
            $createUrl = has_role(['admin', 'medecin']) ? url('?page=consultation-create') : null;
        }
        $this->page('consultations', 'Consultations', $rows, ['dateConsultation' => 'Date', 'patient' => 'Patient', 'medecin' => 'Médecin', 'motif' => 'Motif', 'diagnostique' => 'Diagnostic'], $createUrl, 'Nouvelle consultation');
    }
    ...
    public function exams(): void
    {
        ...
        $createUrl = has_role(['admin', 'medecin']) ? url('?page=exam-create') : null;
        $this->page('exams', 'Examens médicaux', $rows, ['dateExamen' => 'Date', 'patient' => 'Patient', 'typeExamen' => 'Type', 'statut' => 'Statut', 'resultat' => 'Résultat'], $createUrl, 'Demander un examen');
    }
    ...
    public function prescriptions(): void
    {
        ...
        $createUrl = has_role(['admin', 'medecin']) ? url('?page=prescription-create') : null;
        $this->page('prescriptions', 'Prescriptions médicales', $rows, ['datePrescription' => 'Date', 'patient' => 'Patient', 'posologie' => 'Posologie', 'dureeTraitement' => 'Durée', 'statut' => 'Statut'], $createUrl, 'Nouvelle prescription');
    }
    ...
    public function medicaments(): void
    {
        ...
        $createUrl = has_role(['admin', 'medecin', 'pharmacien']) ? url('?page=medicament-create') : null;
        $this->page('medicaments', 'Stock de médicaments', $rows, ['nomCommercial' => 'Nom commercial', 'nomGenerique' => 'Nom générique', 'dosage' => 'Dosage', 'formePharmaceutique' => 'Forme', 'quantiteEnStock' => 'Stock', 'prix' => 'Prix'], $createUrl, 'Nouveau médicament');
    }

    public function medicamentCreate(): void
    {
        require_role(['admin', 'medecin', 'pharmacien']);
    ...
    public function interactions(): void
    {
        ...
        $createUrl = has_role(['admin', 'medecin', 'pharmacien']) ? url('?page=interaction-create') : null;
        $this->page('interactions', 'Interactions médicamenteuses', $rows, ['medicament' => 'Médicament', 'associe' => 'Associé', 'niveau' => 'Niveau', 'description' => 'Description'], $createUrl, 'Nouvelle interaction');
    }
    ...
    {
        require_role(['admin', 'medecin', 'laborantin']);
    ...
    {
        if (has_role('patient')) {
            $stmt = db()->prepare('SELECT r.*, CONCAT(p.prenom, " ", p.nom) patient, u.name medecin FROM rendez_vous r JOIN patients p ON p.id=r.patientId JOIN medecins m ON m.id=r.medecinId JOIN users u ON u.id=m.userId WHERE p.userId = ? ORDER BY r.date');
            $stmt->execute([auth()['id']]);
            $rows = $stmt->fetchAll();
        } else {
    ...
    public function alerts(): void
    {
        ...
        $createUrl = has_role('admin') ? url('?page=alert-create') : null;
        $this->page('alerts', 'Alertes sanitaires', $rows, ['titre' => 'Titre', 'niveau' => 'Niveau', 'statut' => 'Statut', 'description' => 'Description', 'dateDebut' => 'Début'], $createUrl, 'Nouvelle alerte');
    }
