...
class MedicalController
{
    ...
    public function hospitals(): void
    {
        ...
        $this->page('hospitals', 'Hôpitaux', $rows, ['nom' => 'Établissement', 'adresse' => 'Adresse', 'telephone' => 'Téléphone', 'lits' => 'Lits', 'litsOccupes' => 'Occupés', 'occupation' => 'Occupation %', 'services' => 'Services'], '/?page=hospital-create', 'Nouvel hôpital');
    }
    ...
    { $rows = db()->query('SELECT c.*, CONCAT(p.prenom, " ", p.nom) patient, u.name medecin FROM consultations c JOIN patients p ON p.id=c.patientId JOIN medecins m ON m.id=c.medecinId JOIN users u ON u.id=m.userId ORDER BY c.dateConsultation DESC')->fetchAll(); $this->page('consultations', 'Consultations', $rows, ['dateConsultation'=>'Date','patient'=>'Patient','medecin'=>'Médecin','motif'=>'Motif','diagnostique'=>'Diagnostic'], '/?page=consultation-create', 'Nouvelle consultation'); }
    ...
    { $rows = db()->query('SELECT e.*, e.typeExamen, CONCAT(p.prenom, " ", p.nom) patient FROM examens_medicaux e JOIN consultations c ON c.id=e.consultationId JOIN patients p ON p.id=c.patientId ORDER BY e.dateExamen DESC')->fetchAll(); $this->page('exams', 'Examens médicaux', $rows, ['dateExamen'=>'Date','patient'=>'Patient','typeExamen'=>'Type','statut'=>'Statut','resultat'=>'Résultat'], '/?page=exam-create', 'Demander un examen'); }
    ...
    { $rows = db()->query('SELECT p.*, CONCAT(pa.prenom, " ", pa.nom) patient FROM prescriptions_medicales p JOIN consultations c ON c.id=p.consultationId JOIN patients pa ON pa.id=c.patientId ORDER BY p.datePrescription DESC')->fetchAll(); $this->page('prescriptions', 'Prescriptions médicales', $rows, ['datePrescription'=>'Date','patient'=>'Patient','posologie'=>'Posologie','dureeTraitement'=>'Durée','statut'=>'Statut'], '/?page=prescription-create', 'Nouvelle prescription'); }
    ...
    { $rows = db()->query('SELECT * FROM medicaments ORDER BY nomCommercial')->fetchAll(); $this->page('medicaments', 'Stock de médicaments', $rows, ['nomCommercial'=>'Nom commercial','nomGenerique'=>'Nom générique','dosage'=>'Dosage','formePharmaceutique'=>'Forme','quantiteEnStock'=>'Stock','prix'=>'Prix'], '/?page=medicament-create', 'Nouveau médicament'); }
    ...
    { $rows = db()->query('SELECT d.*, m.nomCommercial medicament, d.quantite, d.dateDelivrance FROM delivrances d JOIN medicaments m ON m.id=d.medicamentId ORDER BY d.dateDelivrance DESC')->fetchAll(); $this->page('deliveries', 'Délivrances', $rows, ['dateDelivrance'=>'Date','medicament'=>'Médicament','quantite'=>'Quantité'], '/?page=delivery-create', 'Enregistrer une délivrance'); }
    ...
    { $rows = db()->query('SELECT r.*, CONCAT(p.prenom, " ", p.nom) patient, u.name medecin FROM rendez_vous r JOIN patients p ON p.id=r.patientId JOIN medecins m ON m.id=r.medecinId JOIN users u ON u.id=m.userId ORDER BY r.date')->fetchAll(); $this->page('appointments', 'Rendez-vous', $rows, ['date'=>'Date','patient'=>'Patient','medecin'=>'Médecin','motif'=>'Motif','statut'=>'Statut'], '/?page=appointment-create', 'Nouveau rendez-vous'); }
    ...
    { $rows = db()->query('SELECT * FROM alertes_sanitaires ORDER BY dateDebut DESC')->fetchAll(); $this->page('alerts', 'Alertes sanitaires', $rows, ['titre'=>'Titre','niveau'=>'Niveau','statut'=>'Statut','description'=>'Description','dateDebut'=>'Début'], '/?page=alert-create', 'Nouvelle alerte'); }
