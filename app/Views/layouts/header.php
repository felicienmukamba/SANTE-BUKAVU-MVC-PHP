...
<aside class="sidebar" id="sidebar">
  <a class="sidebar-brand" href="<?= url('?page=dashboard') ?>">
    <img src="<?= asset('brand/logo.png') ?>" alt="Santé+" class="sidebar-brand-logo">
  ...
  <nav class="sidebar-nav">
    ...
    <a class="sidebar-link <?= in_array($currentPage, ['patients','patient-create','patient-edit','patient-view']) ? 'active' : '' ?>" href="<?= url('?page=patients') ?>">
    ...
    <a class="sidebar-link <?= in_array($currentPage, ['medecins','medecin-create','medecin-edit']) ? 'active' : '' ?>" href="<?= url('?page=medecins') ?>">
    ...
    <?php if (has_role(['admin', 'medecin', 'patient'])): ?>
    <a class="sidebar-link <?= in_array($currentPage, ['consultations','consultation-create','consultation-edit']) ? 'active' : '' ?>" href="<?= url('?page=consultations') ?>">
    ...
    <?php if (has_role(['admin', 'medecin'])): ?>
    <a class="sidebar-link <?= in_array($currentPage, ['exams','exam-create','exam-edit']) ? 'active' : '' ?>" href="<?= url('?page=exams') ?>">
    ...
    <?php if (has_role(['admin', 'medecin', 'patient'])): ?>
    <a class="sidebar-link <?= in_array($currentPage, ['prescriptions','prescription-create','prescription-edit']) ? 'active' : '' ?>" href="<?= url('?page=prescriptions') ?>">
    ...
    <?php if (has_role(['admin', 'medecin', 'pharmacien'])): ?>
    <a class="sidebar-link <?= in_array($currentPage, ['medicaments','medicament-create','medicament-edit']) ? 'active' : '' ?>" href="<?= url('?page=medicaments') ?>">
    ...
    <a class="sidebar-link <?= in_array($currentPage, ['deliveries','delivery-create','delivery-edit']) ? 'active' : '' ?>" href="<?= url('?page=deliveries') ?>">
    ...
    <a class="sidebar-link <?= in_array($currentPage, ['interactions','interaction-create','interaction-edit']) ? 'active' : '' ?>" href="<?= url('?page=interactions') ?>">
    ...
    <a class="sidebar-link <?= in_array($currentPage, ['appointments','appointment-create','appointment-edit']) ? 'active' : '' ?>" href="<?= url('?page=appointments') ?>">
      <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="3" y="4" width="18" height="18" rx="2" ry="2"/><line x1="16" y1="2" x2="16" y2="6"/><line x1="8" y1="2" x2="8" y2="6"/><line x1="3" y1="10" x2="21" y2="10"/></svg>
      <span class="sidebar-link-text">Rendez-vous</span>
    </a>

    <?php if (has_role(['admin', 'medecin', 'laborantin'])): ?>
    <a class="sidebar-link <?= in_array($currentPage, ['laboratories','lab-result-create','lab-result-edit']) ? 'active' : '' ?>" href="<?= url('?page=laboratories') ?>">
    ...
    <?php if (has_role('admin')): ?>
    <a class="sidebar-link <?= in_array($currentPage, ['hospitals','hospital-create','hospital-edit']) ? 'active' : '' ?>" href="<?= url('?page=hospitals') ?>">
    ...
    <a class="sidebar-link <?= in_array($currentPage, ['alerts','alert-create','alert-edit']) ? 'active' : '' ?>" href="<?= url('?page=alerts') ?>">
  ...
  <div class="sidebar-footer">
    <div class="sidebar-user">
      <div class="sidebar-avatar"><?= strtoupper(mb_substr(auth()['name'] ?? 'U', 0, 1)) ?></div>
      <div class="sidebar-user-info">
        <div class="sidebar-user-name"><?= e(auth()['name'] ?? 'Utilisateur') ?></div>
        <div class="sidebar-user-role">
          <?= e(match(strtolower((string)(auth()['role'] ?? ''))) {
            'admin' => 'Administrateur',
            'medecin' => 'Médecin',
            'patient' => 'Patient',
            'pharmacien' => 'Pharmacien',
            'laborantin' => 'Laborantin',
            default => ucfirst((string)(auth()['role'] ?? 'Utilisateur'))
          }) ?>
        </div>
      </div>
    </div>
...
<header class="topbar" id="topbar">
  <div class="topbar-left">
    ...
    <div class="d-flex align-items-center gap-2 d-lg-none">
      <img src="<?= asset('brand/logo.png') ?>" alt="Logo" style="height: 28px; width: auto; object-fit: contain;">
      <span class="fw-bold fs-6 text-dark">SANTÉ+</span>
    </div>
  </div>
